-- ============================================
-- WORKSPHERE AI - TEST AUTHENTICATION FLOW (FIXED)
-- ============================================
...
-- Only run verification if the function exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.routines 
    WHERE routine_name = 'verify_database_setup' AND routine_schema = 'public'
  ) THEN
    -- Run verification
    RAISE NOTICE 'Running verification function...';
    PERFORM * FROM verify_database_setup();
  ELSE
    RAISE NOTICE 'Verification function not found, skipping...';
  END IF;
END $$;
